<template>
  <div class="roulette">
    ...
    <transition name="slide-bottom">
      <div v-if="!isSpin" class="roulette__button">
        <ui-button-base
        ...
        />
        <ui-button-base v-if="backButton" v-bind="backButton" />
      </div>
    </transition>
  </div>
</template>
...
const { getCaseItems, openCase } = useCasesStore()
const { receivedItem, musicUrl } = storeToRefs(useCasesStore())
...
const formattedPrice = computed(() =>
  formatePrice(props.currentCase.price || 0),
)
...
async function startAutoScroll(peakSpeedPxS = 800) {
  if (props.currentCase.price > balance.value) {
  ...
  const data = await openCase(props.id, props.currentCase.category === 'robux')
  ...
  const step = (now: number) => {
    ...
    if (progress < 1) {
      ...
      rafId = null
      router.replace({
        path: `/cases/item/${props.id}`,
        ...(props.isOnboarding && { query: { ...route.query, onboarding: 1 } }),
      })
      isSpin.value = false
    }
  }
...
onMounted(async () => {
  ...
  await getCaseItems(props.id, props.currentCase.category === 'robux')
